...
describe('e2e: preview → apply-patch → synthesize', () => {
  it(
    ...
    async () => {
      ...
      expect(synthBody.code).toContain(`{text['login-btn'] ?? "Sign in"}</button>`);
      ...
      expect(synthBody.code).not.toMatch(/bg-\[#185FA5\][^"]*">\{text\['login-btn'\]/);
      // Synthesis contract: header hash binds the generated file to its DSL.
      expect(synthBody.code).toMatch(
