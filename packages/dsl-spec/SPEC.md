...
```
TEXT <id> <x:int> <y:int> "<string>" [size:int] [weight:Weight] [color:color] [align:Align] [max-width:int] [goto:ident] [level:h1..h6] [href:string]
```
...
| level     | no       | `h1`–`h6` ([semantic intent](#semantic-intent)) | none (`<span>`) |
| href      | no       | quoted URL ([semantic intent](#semantic-intent)) | none |
...
```
IMAGE <id> <x:int> <y:int> <w:int> <h:int> <src:string> [fit:Fit] [r:int] [goto:ident] [alt:string] [href:string]
```
...
| r     | no       | int (px) | `0` |
| goto  | no       | screen id ([flow link](#flow-links)) | none |
| alt   | no       | quoted string ([semantic intent](#semantic-intent)) | `""` |
| href  | no       | quoted URL ([semantic intent](#semantic-intent)) | none |
...
```
BUTTON <id> <x:int> <y:int> <w:int> <h:int> "<label>" [variant:ButtonVariant] [state:ElementState] [goto:ident] [href:string]
```
...
<a id="semantic-intent"></a>
**Semantic intent params.** `level:` (TEXT), `alt:` (IMAGE), `href:`
(TEXT/IMAGE/BUTTON), and `role:` (LAYER/STACK/GRID,
`nav|header|main|section|aside|footer`) are **renderer-inert**: pixels never
change. They exist for codegen — `level:h1` emits `<h1>`, `alt:` lands on the
`<img>`, `href:` wraps the element in `<a>`, `role:nav` emits `<nav>` instead
of `<div>` — so synthesized code is semantic, accessible, and SEO-capable.
Visual neutrality of heading tags relies on Tailwind preflight resetting
h1–h6 defaults. Generated components also carry the synthesis contract: a
`// GENERATED by PixelAgent -- dsl-sha256:…` header (verify with
`verifySynthesizedCode(code, dsl)` from `@pixelagent/codegen`) plus `text`
and `on` props keyed by element id so logic lives in a wrapper, never in the
generated file.
...
```
LAYER <id> <x:int> <y:int> <w:int> <h:int> [bg:color] [r:int] [border:width color] [role:Role]
  <child commands>
END
```

`role:` (`nav|header|main|section|aside|footer`) is a renderer-inert
[semantic intent](#semantic-intent) param — codegen emits the matching
semantic tag instead of `<div>`. Also valid on STACK and GRID.
...
| gap       | no       | int (px) | `0` |
| align     | no       | `left` \| `center` \| `right` | `left` |
| role      | no       | container role ([semantic intent](#semantic-intent)) | none (`<div>`) |
